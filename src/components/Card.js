...
function Card() {
    const [defaultCity, setDefaultCity] = useState('nellore');
    ...
    const getData = (city) => {
        axios
            .get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=d8596b1261b43be39522177d29112a96`)
            .then((response) => {
                ...
                setShowWeather(true); // Show weather information
    
                // Log icon URL for debugging
                console.log(`Icon URL: https://openweathermap.org/img/wn/${iconCode}@2x.png`);
            })
            .catch((error) => {
